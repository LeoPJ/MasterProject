...
if __name__ == '__main__':
    ...
    course_list = ['Chinese_12']
    for j in course_list:
        frame_path = f'./resnet18_image_classify/data2/frames/{j}/'
        frame_number = len(os.listdir(frame_path))
        print(frame_path + str(frame_number) + '张图')
        
        close_path = f'./resnet18_image_classify/data2/frames/close/{j}/'
        if not os.path.exists(close_path):
            os.makedirs(close_path)
    
        res_json = []
        for i in range(1, frame_number+1):
            image_path = os.path.join(frame_path, f'Chinese_{i}.jpg')
        ...
        with open(f'./resnet18_image_classify/data2/frames/echarts_result/{j}.txt', 'w') as f:
            for k in res_json:
                f.write(str(k) + ',\n')
